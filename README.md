...
* **現在の課題**: カジュアル対戦時の順番ぎめの際、10秒カウントダウンを行い、すべてのプレイヤーがロールダイスをしなかった場合、ルーム解散、すべてのプレイヤーをロビー画面に戻す実装を行う。
* **今後のタスク**:ランク対戦の設計と実装。カジュアル対戦におけるステージ選択機能。ログイン認証と購入機能。UIを購入するトークンストアの実装。